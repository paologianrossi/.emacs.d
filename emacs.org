...
** Caveats
   ...
   + If on the Mac, use a Cocoa version.  This is mostly for keyboard
     ...
     better on Cocoa.  You could try out [[http://emacsformacosx.com/][Emacs for MacOS X]] or use brew:

   `brew install emacs --with-cocoa --with-ctags --with-glib --with-gnutls --with-imagemagick --with-librsvg)`

* Implementation
** Portability
   Define predicates for platform detection and harmonization. Right
   now I am only interested in MacOS as a special case.
...
** Defining where to look for things
    ...
    - On top of emacs specific stuff, we also need to add
      ...
      various scenarios (for example to reach gpg for encryption).
...
** Personal info
   ...
   - Load what is in ~./private/~ if it's there.  This is useful to
...
** Servers
   ...
   - The other interesting thing is using Emacs as an external
     editor. For example, you can use the [[https://chrome.google.com/webstore/detail/edit-with-emacs/ljobjlafonikaiipfkggjbhkghgicgoh?hl=en][Edit with Emacs]] extension
     for Chrome to edit text areas in Chrome in Emacs.
    #+begin_src emacs-lisp
...
** Visuals
   ...
   - The default font (on a Mac) should be Source Code Pro (available elsewhere), size 14, for my taste.
     #+begin_src emacs-lisp
